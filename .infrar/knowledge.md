...
## Dependencies
express and http-proxy-middleware (existing; server.js). No new dependencies for the share feature — it uses the browser's fetch and existing CSS variables (--muted, --text, --line, --accent). The share endpoint is provided by the orbit-api node and reached only through the same-origin /api proxy; no API host is hardcoded.
