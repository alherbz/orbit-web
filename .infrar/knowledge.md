...
| public/index.html | Static shell: document `<title>` and topbar `<h1>` both read **Orbit Demo 2026**; welcome modal, topbar (logo, `.branch-badge`, `#health-chip`), composer form `#new-task` (fields `#title` and `#priority`), filter nav, and the four board containers `#skeleton` / `#tasks` / `#empty` / `#error` |
...
## Notes
- **The home title is literal markup in two places.** `public/index.html` carries the name twice — the `<head>` `<title>` and the topbar `<h1>` — both now `Orbit Demo 2026`, neither derived at runtime and neither read by `app.js` or `styles.css` (no selector targets the `h1` text; `.brand h1` styles it by position). Renaming the app means editing both lines or the tab and the header drift apart. The welcome modal's body text still says "Welcome to **Orbit**", which is the product name, not the page title.
