...
## Surface
**Exposes** — HTTP server on `PORT` (default `8080`). `GET /health` answers `{ status, service, version }`; `/api/*` is proxied to orbit-api; other paths are static files from `public`.
